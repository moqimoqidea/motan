            ...
            try {
                ...
                for (URL url : originRegistryUrls) {
                    if (NOT_PROCESS_REGISTRY_PROTOCOLS.contains(url.getProtocol())) {
                        ...
                        URL meshRegistryUrl = URL.valueOf(url.toFullStr());
                        meshRegistryUrl.setProtocol(MeshConstants.MESH_PROTOCOL_NAME);
                        meshRegistryUrl.addParameter(MeshConstants.MESH_PROXY_MODE_KEY, proxyConfig.get(MODE_KEY));
